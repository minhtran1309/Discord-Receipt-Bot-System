...
class TestTotalCostMonthlySheet:
    ...
    def test_get_row_names(self, sheets_service):
        ...
        try:
            ...
            assert len(category_names) > 0, "No categories found in total_cost_monthly sheet"
    ...
    def test_get_column_names(self, sheets_service):
        ...
        try:
            ...
            # Assertions
            assert category_header == "Category", f"Expected first column 'Category', got '{category_header}'"
            assert len(month_columns) > 0, "No month columns found in total_cost_monthly sheet"
    ...
    def test_get_complete_table_structure(self, sheets_service):
        ...
        try:
            ...
            category_rows = [row[0] for row in all_values[1:]] if len(all_values) > 1 else []

            print("\n" + "="*80)
            print("TOTAL_COST_MONTHLY - COMPLETE TABLE STRUCTURE")
            print("="*80)
            print(f"Dimensions: {len(category_rows)} categories × {len(month_columns)} months")
    ...
    def test_find_specific_month(self, sheets_service):
        ...
        try:
            ...
            assert month_col >= 2, "Month column should be at least column 2 (after Category)"
